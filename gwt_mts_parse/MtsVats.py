...
class VATS:
    ...
    async def get_list_trunks(self, page):
        ...
        online_trunks = set(re.findall(r"C.{5}',2,0,0", response)) # '2,0,0,\d,8,\d,\d,0,\d,0,0,0'
    ...
    async def query(self, url, payload=None, headers=None, method='POST'):
        if headers and payload:
            response = await self.session.request(method=method, url=url, data=payload, headers=headers, ssl=False)
        elif headers and not payload:
            response = await self.session.request(method=method, url=url, headers=headers, ssl=False)
    ...
    def trunk_list_action(self, sim_list, action):
        ...
        for num in sim_list:
            ...
            if str_num in all_trunks:
                ...
                if action == 'add':
                    payload = TRUNK_ADD_SIP.format(identify_line=trunk['trunk_identify_line'],
                                                   inner_link=trunk['trunk_inner_link'],
                                                   sip_obj=trunk['trunk_sip_obj'],
                                                   contract_link=self.contract_url_abonents)
                if action == 'del':
                    payload = TRUNK_DESTROY_SIP.format(trunk['trunk_inner_link'])
                query_list.append(loop.create_task(self.query(url=url, payload=payload, headers=self.headers)))
